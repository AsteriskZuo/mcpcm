...
- [x] 1.1 Create `app/src/telemetry.ts` with typed ingest payload and response/error handling helpers.
...
- [x] 2.1 Refactor `app/src/commands/add.ts` to track per-target outcomes (added, replaced, skipped, failed) per `mcp_name`.
...
- [x] 4.3 Run `npm test` and `npm run type-check` in `app/` and update documentation/comments where behavior expectations changed.
